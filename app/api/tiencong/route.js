...
export async function GET() {
  try {
    ...
    return NextResponse.json({ message: "Lấy tiền công thành công", data: list });
  } catch (error) {
 ...
 * POST /api/tien-cong
 * Thêm tiền công mới
 * Body: { TenTienCong, GiaTienCong }
 */
export async function POST(req) {
  try {
    const data = await req.json();
    const tienCong = await lapTienCong(data);
    return NextResponse.json({ message: "Tạo tiền công thành công", data: tienCong });
  } catch (error) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
